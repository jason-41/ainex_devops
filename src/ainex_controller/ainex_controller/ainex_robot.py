...
class AinexRobot():
    def __init__(self, node: Node, model: AiNexModel, dt: float, sim: bool = True):
        ...
        # Dynamically set joint position limits based on actual joint count
        nq = self.robot_model.model.nq
        self.joint_pos_limits_lower = np.full(nq, -3.14)  # Default lower limits
        self.joint_pos_limits_upper = np.full(nq, 3.14)   # Default upper limits
        
        # Set specific limits based on joint names
        for i, joint_name in enumerate(self.joint_names):
            if 'sho_roll' in joint_name:
                self.joint_pos_limits_lower[i] = -1.57
                self.joint_pos_limits_upper[i] = 1.57
            elif 'el_pitch' in joint_name:
                self.joint_pos_limits_lower[i] = -1.57
                self.joint_pos_limits_upper[i] = 1.57
        
        self.node.get_logger().info(f"Joint count: {nq}, Joint names: {self.joint_names}")
    ...
    def update(self, v_cmd_left: np.ndarray, v_cmd_right: np.ndarray, dt: float):
        ...
        q_next = self.q + self.v * dt
        
        # Check and limit joint positions
        for i in range(len(q_next)):
            if q_next[i] < self.joint_pos_limits_lower[i]:
                q_next[i] = self.joint_pos_limits_lower[i]
                self.v[i] = 0.0  # Stop joint motion
                self.node.get_logger().warn(f"Joint {i} ({self.joint_names[i]}) reached lower limit")
            elif q_next[i] > self.joint_pos_limits_upper[i]:
                q_next[i] = self.joint_pos_limits_upper[i]
                self.v[i] = 0.0  # Stop joint motion
                self.node.get_logger().warn(f"Joint {i} ({self.joint_names[i]}) reached upper limit")
    ...
    def send_cmd(self, q_cmd: np.ndarray, dt: float):
        ...
        q_cmd[self.robot_model.get_joint_id('r_sho_pitch')] *= 1.0 #This was a bug before! should be +1.0
