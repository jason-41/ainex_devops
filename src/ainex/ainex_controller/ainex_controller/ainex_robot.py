...
class AinexRobot():
    def __init__(self, node: Node, model: AiNexModel, dt: float, sim: bool = True):
        ...
        self.joint_pos_limits_lower = np.full(
            nq, -3.14)  # Default lower limits
        self.joint_pos_limits_upper = np.full(
            nq, 3.14)   # Default upper limits
        ...
        self.node.get_logger().info(
            f"Joint count: {nq}, Joint names: {self.joint_names}")
        ...
        self.joint_states_pub = self.node.create_publisher(
            JointState, 'ainex_joint_states', 10)
        ...
        for i in range(len(q_next)):
            if q_next[i] < self.joint_pos_limits_lower[i]:
                ...
                self.node.get_logger().warn(
                    f"Joint {i} ({self.joint_names[i]}) reached lower limit")
            elif q_next[i] > self.joint_pos_limits_upper[i]:
                q_next[i] = self.joint_pos_limits_upper[i]
                self.v[i] = 0.0  # Stop joint motion
                self.node.get_logger().warn(
                    f"Joint {i} ({self.joint_names[i]}) reached upper limit")
    ...
    def read_joint_positions_from_robot(self):
        ...
        q_real = np.array(
            self.joint_controller.getJointPositions(self.joint_names))
